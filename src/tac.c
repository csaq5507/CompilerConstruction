...
static void
push_tac_end_single_expression(struct mCc_ast_single_expression *expression,
			       tac_list *tac_end);
static void push_tac_end_expression(struct mCc_ast_expression *expression,
				    tac_list *tac_end);
static void set_tac_literal(tac_list *elem,
...
static enum mCc_tac_operation_type
ast_to_tac_op_type(enum mCc_ast_binary_op op);
enum mCc_tac_operation_type
get_inverse_operator(enum mCc_tac_operation_type type);
...
static void
tac_single_expression_unary(struct mCc_ast_single_expression *expression,
...
static struct mCc_ast_compound_stmt *
delete_stmt_from_list(struct mCc_ast_compound_stmt *c_stmt, int position);
			    ...
			    struct mCc_ast_single_expression *expression)
{
	...
	case (MCC_AST_LITERAL_TYPE_STRING):
		...
		elem->s_literal = copy_string(expression->literal->s_value);
		break;
	}
}
...
static void negate_or(struct mCc_ast_expression *expression)
{
	...
	if (tac_end->type == MCC_TAC_ELEMENT_TYPE_LABEL) {
		...
		if (expression->rhs->type == MCC_AST_EXPRESSION_TYPE_SINGLE) {
			expression->rhs->single_expr->tac_end =
				expression->tac_end;
		}
		tac_end->prev = NULL;
		tac_end->next = NULL;
...
static void negate_tac(struct mCc_ast_expression *expression)
{
	...
	if (expression->op == MCC_AST_BINARY_OP_OR) {
		...
		if (operation->type == MCC_TAC_ELEMENT_TYPE_BINARY)
			operation->binary_op_type =
				get_inverse_operator(operation->binary_op_type);
		else if (operation->type == MCC_TAC_ELEMENT_TYPE_COPY_LITERAL)
			operation->b_literal = !operation->b_literal;
		else if (operation->type
			 == MCC_TAC_ELEMENT_TYPE_COPY_IDENTIFIER) {
			if (expression_end->type == MCC_TAC_ELEMENT_TYPE_LABEL
			    || expression_end->type
				       == MCC_TAC_ELEMENT_TYPE_CONDITIONAL_JUMP) {
				tac_list *unary_not = tac_new_list();
				unary_not->type = MCC_TAC_ELEMENT_TYPE_UNARY;
				unary_not->unary_op_type =
					MCC_TAC_OPERATION_TYPE_FAC;
				unary_not->identifier1 =
					new_string("reg_%d", v_counter++);
				unary_not->unary_identifier =
					copy_string(operation->identifier1);
				...
				if (expression_end->type
				    == MCC_TAC_ELEMENT_TYPE_LABEL) {
					free(expression_end->prev->identifier1);
					expression_end->prev->identifier1 =
						copy_string(
							unary_not->identifier1);
				} else {
					free(expression_end->identifier1);
					expression_end->identifier1 =
						copy_string(
							unary_not->identifier1);
				}
			} else {
				...
				if (expression->single_expr != NULL)
					expression->single_expr->tac_end =
						unary_not;
			}
		}
	}
...
static void negate_tac_single(struct mCc_ast_single_expression *expression)
{
	...
	if (expression->type == MCC_AST_SINGLE_EXPRESSION_TYPE_PARENTH) {
		...
		unary_not->unary_identifier = copy_string(tmp->identifier1);
			    ...
			    struct mCc_tac_list *elem)
{
	...
	if (expression->unary_operator == MCC_AST_UNARY_OP_NEGATION) {
		...
		expression->tac_end = elem;
		   ...
		   && expression->unary_expression->type
			      == MCC_AST_EXPRESSION_TYPE_SINGLE
		   && expression->unary_expression->single_expr->type
		...
		if (expression->unary_expression->type
		    == MCC_AST_EXPRESSION_TYPE_SINGLE) {
			negate_tac_single(
				expression->unary_expression->single_expr);
			...
			expression->tac_start =
				expression->unary_expression->tac_start;
			expression->tac_end =
				expression->unary_expression->tac_end;
		}
...
static void generate_tac_operation_or(struct mCc_ast_expression *expression)
{
	...
	if (rhs_last->type == MCC_TAC_ELEMENT_TYPE_LABEL) {
		...
		jump_rhs->identifier1 = copy_string(
			((struct mCc_tac_list *)expression->rhs->tac_end)
				->identifier1);
		jump_rhs->jump = NULL;
		label->type = MCC_TAC_ELEMENT_TYPE_LABEL;
		label->identifier1 = new_string("L%d", l_counter++);
	...
	if (lhs_last->type == MCC_TAC_ELEMENT_TYPE_LABEL) {
		...
		lhs_last->next = NULL;
		lhs_last->prev = NULL;
		remove_jump(expression->lhs->tac_start,
			    expression->lhs->tac_end, lhs_last->identifier1);
		if (expression->lhs->type
		    == MCC_AST_SINGLE_EXPRESSION_TYPE_PARENTH) {
			expression->lhs->expression->tac_end =
				expression->lhs->tac_end;
		}
		...
		add_jump(expression->lhs->tac_start, lhs_last->prev, label_and);
		...
		jump_lhs->identifier1 = copy_string(
			((struct mCc_tac_list *)expression->lhs->tac_end)
				->identifier1);
		jump_lhs->jump = label;
	}
	...
	if (expression->lhs->type == MCC_AST_SINGLE_EXPRESSION_TYPE_PARENTH) {
		...
		if (expression->lhs->expression->type
		    == MCC_AST_EXPRESSION_TYPE_BINARY)
			expression->lhs->expression->rhs->tac_end =
				expression->lhs->tac_end;
	}
	...
	if (lhs_last->type != MCC_TAC_ELEMENT_TYPE_LABEL) {
		((struct mCc_tac_list *)expression->lhs->tac_end)->next =
			jump_lhs;
		jump_lhs->prev = expression->lhs->tac_end;
		jump_lhs->next = expression->rhs->tac_start;
		((struct mCc_tac_list *)expression->rhs->tac_start)->prev =
			jump_lhs;
	} else {
		((struct mCc_tac_list *)expression->lhs->tac_end)->next =
			expression->rhs->tac_start;
		((struct mCc_tac_list *)expression->rhs->tac_start)->prev =
			expression->lhs->tac_end;
	}
	...
	if (rhs_last->type != MCC_TAC_ELEMENT_TYPE_CONDITIONAL_JUMP) {
		((struct mCc_tac_list *)expression->rhs->tac_end)->next =
			jump_rhs;
			       ...
			       tac_list *tac_end)
{
	...
	case (MCC_AST_SINGLE_EXPRESSION_TYPE_IDENTIFIER_EX):
		push_tac_end_expression(expression->identifier_expression,
					tac_end);
		break;
				    ...
				    tac_list *tac_end)
{
	...
	case (MCC_AST_EXPRESSION_TYPE_SINGLE):
		push_tac_end_single_expression(expression->single_expr,
					       tac_end);
		break;
...
static void add_copy_rhs(struct mCc_ast_expression *expression)
{
	...
	tac_list *new_copy_rhs1 = tac_new_list();
	tac_list *new_copy_rhs2 = tac_new_list();
	new_copy_rhs2->type = MCC_TAC_ELEMENT_TYPE_COPY_IDENTIFIER;
	new_copy_rhs2->identifier1 = new_string("reg_%d", v_counter++);
	new_copy_rhs2->copy_identifier =
		copy_string(new_param_rhs->identifier1);
	new_copy_rhs1->type = MCC_TAC_ELEMENT_TYPE_COPY_IDENTIFIER;
	...
	new_copy_rhs2->prev = new_copy_rhs1;
	new_copy_rhs2->next = temp_rhs_end->next;
...
static void add_copy_lhs(struct mCc_ast_expression *expression)
{
	...
	new_copy_lhs2->copy_identifier =
		copy_string(new_param_lhs->identifier1);
...
static void tac_expression(struct mCc_ast_expression *expression, void *data)
{
	...
	if (expression->type == MCC_AST_EXPRESSION_TYPE_SINGLE) {
		...
		enum mCc_tac_operation_type op_type =
			ast_to_tac_op_type(expression->op);
		...
		if (op_type == MCC_TAC_OPERATION_TYPE_OR) {
			...
			if (temp_rhs_end->type == MCC_TAC_ELEMENT_TYPE_BINARY) {
				if (elem->binary_op_type
					    == MCC_TAC_OPERATION_TYPE_MINUS
				    && temp_rhs_end->binary_op_type
					       == MCC_TAC_OPERATION_TYPE_MINUS) {
					temp_rhs_end->binary_op_type =
						MCC_TAC_OPERATION_TYPE_PLUS;
				}
...
static void generate_parameter_calls(struct mCc_ast_call_expr *expression)
{
	...
	if (expression->arguments != NULL) {
		...
		if (expression->d_type == MCC_AST_TYPE_VOID) {
			for (int i = expression->arguments->counter - 1; i >= 0;
			     i--) {
				tac_list *actual =
				...
				ret_elem->type =
					MCC_TAC_ELEMENT_TYPE_PARAMETER_SETUP_CALL;

				ret_elem->identifier1 =
					copy_string(actual->identifier1);
				ret_elem->param_size = 1; // default only arrays
							  // have another size
...
static void tac_ret_stmt(struct mCc_ast_ret_stmt *stmt, void *data)
{
	...
	if (stmt->expression != NULL) {
		...
		if ((stmt->d_type == MCC_AST_TYPE_BOOL
			...
			label_false->identifier1 =
				new_string("L%d", l_counter++);
			...
			elem_false->identifier1 =
				new_string("reg_%d", v_counter++);

			elem_true->type = MCC_TAC_ELEMENT_TYPE_COPY_LITERAL;
			elem_true->literal_type = MCC_TAC_LITERAL_TYPE_BOOL;
			elem_true->b_literal = true;
			elem_true->identifier1 =
				copy_string(elem_false->identifier1);

			if (stmt->d_type == MCC_AST_TYPE_BOOL
			    && (expression_end->type
					== MCC_TAC_ELEMENT_TYPE_BINARY
				|| expression_end->type
					   == MCC_TAC_ELEMENT_TYPE_UNARY)) {
				tac_list *jump_false = tac_new_list();
				tac_list *jump = tac_new_list();
				jump_false->type =
					MCC_TAC_ELEMENT_TYPE_CONDITIONAL_JUMP;
				jump_false->identifier1 = copy_string(
					expression_end->prev->identifier1);
...
static void bool_operation_assignment(struct mCc_ast_assignment *stmt)
{
	...
	if (expression_end->type == MCC_TAC_ELEMENT_TYPE_UNARY
	    || expression_end->type == MCC_TAC_ELEMENT_TYPE_BINARY) {
		jump_false->identifier1 =
			copy_string(expression_end->identifier1);
	} else {
		jump_false->identifier1 =
			copy_string(expression_end->prev->identifier1);
	}
	jump_false->jump = label_false;
	...
	if (stmt->numerator == NULL) {
		...
		elem_false->b_literal = false;
		elem_false->identifier1 =
			copy_string(stmt->identifier->renamed);
		...
		elem_store->identifier1 =
			copy_string(stmt->identifier->renamed);
		elem_store->identifier2 =
			copy_string(numerator_end->identifier1);
...
static void bool_assignment(struct mCc_ast_assignment *stmt)
{
	...
	if (expression_end->type == MCC_TAC_ELEMENT_TYPE_BINARY
		...
		if (stmt->numerator == NULL) {
			...
			tac_list *expression_start =
				stmt->expression->tac_start;
...
static bool handle_empty_if_stmt(struct mCc_ast_if_stmt *stmt)
{
	...
	if (stmt->else_statement == NULL) {
		...
		if (stmt->statement->tac_start == stmt->statement->tac_end
			...
			return true;
		} else if (stmt->statement->tac_start
			   == stmt->statement->tac_end) {
			mCc_tac_delete(stmt->statement->tac_start);
			if (stmt->else_statement->type != MCC_AST_IF_STMT) {
				...
				mCc_tac_delete(stmt->expression->tac_start);
				stmt->tac_start = stmt->else_statement->if_stmt
							  ->tac_start;
				stmt->tac_end =
					stmt->else_statement->if_stmt->tac_end;
				stmt->expression = stmt->else_statement->if_stmt
							   ->expression;
				stmt->statement = stmt->else_statement->if_stmt
							  ->statement;
				stmt->else_statement =
					stmt->else_statement->if_stmt
						->else_statement;
				return true;
			}
		} else {
			if (stmt->else_statement->tac_start
			    == stmt->else_statement->tac_end) {
				mCc_tac_delete(stmt->else_statement->tac_start);
				stmt->else_statement = NULL;
			}
...
static void tac_if_stmt(struct mCc_ast_if_stmt *stmt, void *data)
{
	...
	if (temp_expression_end->type == MCC_TAC_ELEMENT_TYPE_BINARY
		...
		jump_false->identifier1 =
			copy_string(temp_expression_end->identifier1);
	...
	if (stmt->else_statement != NULL) {
		...
		add_jump(stmt->tac_start, stmt->tac_end, label);
	} else {
		free(jump);
		tac_list *temp_stmt_start = stmt->statement->tac_start;
		tac_list *temp_stmt_end = stmt->statement->tac_end;
		...
		stmt->tac_end = label_end;
		add_jump(stmt->tac_start, stmt->tac_end, label_end);
	}
}
...
static void tac_while_stmt(struct mCc_ast_while_stmt *stmt, void *data)
{
	...
	if (stmt->statement->tac_start == stmt->statement->tac_end) {
		...
		mCc_tac_delete(stmt->expression->tac_start);
		mCc_tac_delete(stmt->statement->tac_start);
	} else if (((tac_list *)stmt->expression->tac_end)->type
			   == MCC_TAC_ELEMENT_TYPE_BINARY
		   || ((tac_list *)stmt->expression->tac_end)->type
			      == MCC_TAC_ELEMENT_TYPE_UNARY
		   || ((tac_list *)stmt->expression->tac_end)->type
			      == MCC_TAC_ELEMENT_TYPE_COPY_LITERAL) {
	...
	case (MCC_TAC_ELEMENT_TYPE_COPY_LITERAL):
		...
		case (MCC_TAC_LITERAL_TYPE_BOOL):
			if (current->b_literal)
				fprintf(out, "COPY: %s = true",
					current->identifier1);
			else
				fprintf(out, "COPY: %s = false",
					current->identifier1);
			break;
