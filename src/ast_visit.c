				      ...
				      struct mCc_ast_visitor *visitor)
{
	...
	visit_for(visitor->order, f->counter)
	{
				     ...
				     struct mCc_ast_visitor *visitor)
{
	...
	visit(f->identifier, visitor->identifier, visitor);

	visit_if_pre_order(f->params, visitor->parameter, visitor);
	mCc_ast_visit_parameter(f->params, visitor);
	visit_if_post_order(f->params, visitor->parameter, visitor);
			     ...
			     struct mCc_ast_visitor *visitor)
{
	...
	visit_for(visitor->order, param->counter)
	{
		visit_if_pre_order(&param->declaration[i], visitor->decl_stmt,
				   visitor);
		mCc_ast_visit_decl_stmt(&param->declaration[i], visitor);
		visit_if_post_order(&param->declaration[i], visitor->decl_stmt,
				    visitor);
	}
}
				 ...
				 struct mCc_ast_visitor *visitor)
{
	...
	visit_for(visitor->order, c_stmt->counter)
	{
		/* TYPE */
		visit_if_pre_order(&c_stmt->statements[i], visitor->statement,
				   visitor);
		mCc_ast_visit_stmt_statement(&c_stmt->statements[i], visitor);
		visit_if_post_order(&c_stmt->statements[i], visitor->statement,
				    visitor);
	}
}
	...
	struct mCc_ast_visitor *visitor)
{
	...
	case (MCC_AST_SINGLE_EXPRESSION_TYPE_LITERAL):
		visit_if_pre_order(expression->literal, visitor->literal,
				   visitor);
		mCc_ast_visit_literal(expression->literal, visitor);
		visit_if_post_order(expression->literal, visitor->literal,
				    visitor);
		break;
	...
	case (MCC_AST_SINGLE_EXPRESSION_TYPE_IDENTIFIER_EX):
		...
		visit_if_pre_order(expression->identifier_expression,
				   visitor->expression, visitor);
		mCc_ast_visit_expression(expression->identifier_expression,
					 visitor);
		visit_if_post_order(expression->identifier_expression,
				    visitor->expression, visitor);
	...
	case (MCC_AST_SINGLE_EXPRESSION_TYPE_UNARY_OP):
		visit_if_pre_order(expression->unary_expression,
				   visitor->expression, visitor);
		mCc_ast_visit_expression(expression->unary_expression, visitor);
		visit_if_post_order(expression->unary_expression,
				    visitor->expression, visitor);
		break;
	case (MCC_AST_SINGLE_EXPRESSION_TYPE_PARENTH):
		visit_if_pre_order(expression->expression, visitor->expression,
				   visitor);
			    ...
			    struct mCc_ast_visitor *visitor)
{
	...
	visit_for(visitor->order, argument->counter)
	{

		visit_if_pre_order(&argument->expression[i],
				   visitor->expression, visitor);
		mCc_ast_visit_expression(&argument->expression[i], visitor);
		visit_if_post_order(&argument->expression[i],
				    visitor->expression, visitor);
	}
}
				   ...
				   struct mCc_ast_visitor *visitor)
{
	...
	if (expression->arguments != NULL) {
		visit_if_pre_order(expression->arguments, visitor->argument,
				   visitor);
		mCc_ast_visit_argument(expression->arguments, visitor);
		visit_if_post_order(expression->arguments, visitor->argument,
				    visitor);
	}
}
