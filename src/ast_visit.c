			      ...
			      struct mCc_ast_visitor *visitor)
{
	...
	case MCC_AST_EXPRESSION_TYPE_SINGLE:
		visit_if_pre_order(expression, visitor->expression_single,
				   visitor);
		mCc_ast_visit_single_expression(expression->single_expr,
						visitor);
		visit_if_post_order(expression, visitor->expression_single,
				    visitor);
		break;

	case MCC_AST_EXPRESSION_TYPE_BINARY:
		visit_if_pre_order(expression, visitor->expression_binary,
				   visitor);
		mCc_ast_visit_single_expression(expression->lhs, visitor);
		mCc_ast_visit_expression(expression->rhs, visitor);
		visit_if_post_order(expression, visitor->expression_binary,
				    visitor);
		break;
	}
...
void mCc_ast_visit_single_expression(
	struct mCc_ast_single_expression *single_expression,
	struct mCc_ast_visitor *visitor)
{
	...
	visit_if_pre_order(single_expression, visitor->single_expression,
			   visitor);

	switch (single_expression->type) {
	case MCC_AST_SINGLE_EXPRESSION_TYPE_CALL_EXPR:
		visit(single_expression, visitor->single_expression_call,
		      visitor);
		break;

	case MCC_AST_SINGLE_EXPRESSION_TYPE_IDENTIFIER:
		visit(single_expression, visitor->single_expression_identifier,
		      visitor);
		break;
	case MCC_AST_SINGLE_EXPRESSION_TYPE_IDENTIFIER_EX:
		visit(single_expression,
		      visitor->single_expression_identifier_ex, visitor);
		break;

	case MCC_AST_SINGLE_EXPRESSION_TYPE_PARENTH:
		visit(single_expression, visitor->single_expression_parenth,
		      visitor);
		break;

	case MCC_AST_SINGLE_EXPRESSION_TYPE_UNARY_OP:
		visit(single_expression, visitor->single_expression_unary_op,
		      visitor);
		break;

	case MCC_AST_SINGLE_EXPRESSION_TYPE_LITERAL:
		visit(single_expression, visitor->single_expression_literal,
		      visitor);
		break;
	}

	visit_if_post_order(single_expression, visitor->single_expression,
			    visitor);
}
